...
module.exports = {
        ...
        return extend(DrawBlock, {
            ...
            draw(build){
                ...
                Draw.draw(Layer.blockOver, () => {
                    Draw.reset();
                    Drawf.construct(build, this.run(build), 0, build.progress, build.warmup, build.totalProgress);
                });
        ...
        return extend(DrawBlock, {
            ...
            draw(build){
                ...
                Draw.draw(Layer.block, () => {
                    Drawf.construct(build.x, build.y, build.block.outputItems[0].item.fullIcon, this.color, 0, build.progress, build.warmup, build.totalProgress);
    ...
    DynamicDrawBuilding(color, runnable){
        return extend(DrawBlock, {
            color: color,
            run: runnable,
            ...
            draw(build){
                const region = this.run(build);
    
                Draw.rect(this.bottom, build.x, build.y);
    
                if(region){
                    Draw.draw(Layer.block, () => {
                        Drawf.construct(build.x, build.y, region, this.color, 0, build.progress, build.warmup, build.totalProgress);
